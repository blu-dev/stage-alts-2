...
local normal_tab_main_update = function()
    if check_all_previews_enabled() == false then
        if UiScriptPlayer.invoke("is_hand_interpolated_moving") == false then
            ...
            local count = Alts.get_panel_alt_count(panel_id, stage_previews[current_selected_preview + 1].form_type_)
            if count == 3 and not did_set_alt then
                local texture_idx = Alts.get_alt_texture_index(panel_id,
                    stage_previews[current_selected_preview + 1].form_type_, 3)
                if texture_idx >= 0 then
                    Alts.send_message("texture: " .. tostring(texture_idx))
